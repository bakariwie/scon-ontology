...
Canonical file : scon-ontology-1.1.0.owl  (RDF/XML; opens directly in Protege 5.6+)
...
systematic literature review of ontologies in school counselling
(Discover Artificial Intelligence, advance online publication), which
...
---------------------------------------------------------------------
  Classes ............................. 447
    of which domain classes ........... 441
    of which defined (equivalent) ....  6  (automatic classification)
  Object properties (SCON-native) ..... 52
  Data properties (SCON-native) ....... 21
  Annotation properties ...............  5  (module, provenance,
                                        competencyQuestion + IAO
                                        definition + dcterms:source)
  Named individuals (seed ABox) ....... 16
  Competency questions (annotations) .. 36  (12 per ASCA domain)
  Term definitions (IAO:0000115) ...... 447 (every class defined)
  SubClassOf axioms ................... 468
  AllDisjointClasses axioms ...........  8
  AllDifferent axioms .................  1  (four risk levels)
  Existential restrictions ............ 12
  Property chains .....................  1
  Functional properties ...............  7
  Transitive properties ...............  2
  Inverse object properties ...........  4
...
Logical reuse (term-level, verified IRIs):
  ...
  IAO:0000115 definition                     (textual definitions)
...
Reasoning-relevant axiomatisation:
  * Signature rule: Intervention SubClassOf addresses some StudentIssue.
  * Domain-targeting: each of AcademicIntervention, CareerIntervention,
    and SocialEmotionalIntervention targetsDomain the matching
    CounsellingDomain; the three intervention domains are disjoint.
  * Trustworthiness: CounsellingChatbot SubClassOf groundedInKB some
    CounsellingKnowledgeBase.
  * Disjoint sibling sets: the three ASCA domains; MTSS tiers; the Big
    Five traits; the CASEL competencies; the three intervention
    domains; the three student-issue types; the four disposition
    families; the five emotion episodes; Person vs Organisation.
    The four risk levels are individuals held pairwise distinct by an
    AllDifferent axiom.
  * Defined classes (necessary & sufficient) for automatic
    classification (see Section 5).
  * Property axioms: addresses/isAddressedBy inverse; isRiskIndicatorOf
    (inverse of hasRiskIndicator) and hasRecommendedIntervention
    (inverse of recommendedFor); property chain hasRiskIndicator o
    hasRiskLevel -> hasStudentRiskLevel; hasRiskLevel and assignedTier
    functional; precedes and partOfTrajectory transitive.
...
Population  : LLM-assisted (Claude, Anthropic) under human expert
  ...
  scon:populationProvenance with one of:
    ...
    author-vocabulary (defined class)
...
Profile     : OWL 2 DL. Reasoner: HermiT (hypertableau).
...
---------------------------------------------------------------------
1) Open scon-ontology-1.1.0.owl in Protege 5.6+.
2) Reasoner -> HermiT -> Start reasoner. Expected: CONSISTENT.
   The seed ABox drives automatic classification. For student_S1 (who
   hasRiskIndicator a ChronicAbsence indicator whose hasRiskLevel is
   riskLevel_high, and assignedTier tier2), the reasoner infers:
     - AtRiskStudent
     - ChronicallyAbsentStudent
     - HighRiskStudent
     - StudentReceivingTieredSupport
     - hasStudentRiskLevel riskLevel_high  (via the property chain)
   and intervention_int1 is inferred to be a RecommendedIntervention.
   Also:
...
3) DL Query tab (class expressions), e.g.:
     HighRiskStudent
     Intervention and (recommendedFor some Student)
...
4) SPARQL: use the SNAP SPARQL QUERY tab (Window -> Tabs), which
   ...
   "SPARQL Query" tab sees only asserted triples. Example:
     ...
     SELECT ?student WHERE { ?student a scon:HighRiskStudent }

   (Returns student_S1 only under the reasoner - a direct
   demonstration of inference.) A fuller reasoning-test query pack is
   provided in competency-questions/ in the repository.

Per-release quality checks recommended: HermiT (consistency,
classification), OOPS! (pitfalls), FOOPS! (FAIR), OntoClean
(meta-properties), and CQ pass/fail testing.
...
Please cite the ontology as:
  ...
  Counselling Ontology Network (Version 1.1.0) [OWL ontology].
...
organisation, candidate-axiom generation, class definition drafting,
and documentation, under the human adjudication and HermiT
consistency-gate workflow described above. All committed content is
